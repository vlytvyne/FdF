...
void 	define_len_padding_flat(t_conf *conf)
{
	...
	max_horizontal = conf->map_width - 1 != 0 ? conf->window_width / (conf->map_width - 1) : 50;
	max_vertical = conf->map_height - 1 != 0 ? conf->window_height / (conf->map_height - 1) : 50;
	if (max_horizontal > max_vertical)
	{
		conf->line_len = max_vertical > 2 ? max_vertical - 1 : max_vertical;
		conf->padding_flat.x = (conf->window_width - (conf->line_len * (conf->map_width - 1))) / 2;
		conf->padding_flat.y = (conf->window_height - (conf->line_len * (conf->map_height - 1))) / 2;
	}
	else
	{
...
void	define_padding_iso(t_conf *conf)
{
	...
	padding = (conf->window_width - distance) / 2;
	...
	distance = second_vertex - first_vertex;
	padding = (conf->window_height - distance) / 2;
